...
public class EasyEventBus {
    private static EasyEventBus manager;
     ...
     * @param msg
     */
    private void postMsg(String key,Message msg) {
        OnEventMessageListener listener = listenerHashMap.get(key);
        if(listener!=null && msg!=null && msg.obj!=null){
            listener.onMsg(msg.obj);
        }else{
            Log.d("CHEN", "EasyEventBus error : 没有找到目标类，类名："+c.getName()+"\n");
        }
    }

    public static EasyEventBus getInstance() {
        if (manager == null) {
            synchronized (EasyEventBus.class) {
                if (manager == null) {
                    manager = new EasyEventBus();
                }
            }
        }
        return manager;
     ...
     * @param
     */
    public void unregisterEvent(Class c) {
     ...
     * @param o 发送的内容
     */
    public <T> void sendMsg(Class c, T o) {
        this.c = c;
